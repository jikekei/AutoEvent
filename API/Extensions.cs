...
public static class Extensions
{
    ...
    public static void GiveLoadout(this Player player, Loadout loadout, LoadoutFlags flags = LoadoutFlags.None)
    {
        ...
        {
            ApplyStamina(player, loadout.Stamina);
        }
        else
        {
    ...
    private static void ApplyStamina(Player player, float amount)
    {
        ...
        try
        {
            var type = stamina.GetType();
            var modify = type.GetMethod("ModifyAmount", new[] { typeof(float) });
            if (modify != null)
            {
                modify.Invoke(stamina, new object[] { amount });
                return;
            }

            var add = type.GetMethod("AddAmount", new[] { typeof(float) });
            if (add != null)
            {
                add.Invoke(stamina, new object[] { amount });
                return;
            }
        }
        catch (Exception e)
        {
            DebugLogger.LogDebug("An error occured while applying stamina.", LogLevel.Warn, true);
